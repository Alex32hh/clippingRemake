...
try {
        ...
        if(str_pad((explode(':',date('H:i'))[0]+1), 2, '0', STR_PAD_LEFT) > explode(':',$row['frequancia'])[0]){
            
        $upload = "UPDATE alertas SET activado='' WHERE id='".$row['id']."'";
        if ($db->query($upload) === TRUE)
         ...
         // echo str_pad((explode(':',date('H:i'))[0]+1), 2, '0', STR_PAD_LEFT).'-'.explode(':',$row['frequancia'])[0]."<br>";
                
          if($row['frequancia'] == '00' || $row['frequancia'] == '12' && date('h') == 12 || $row['frequancia'] == '60' && date('N') == 1 || $row['frequancia'] == '225' && date('n') == 1 || date('H:i') == $row['frequancia'] ||  str_pad((explode(':',date('H:i'))[0]+1), 2, '0', STR_PAD_LEFT) == explode(':',$row['frequancia'])[0])
             if($row['activado'] == ''){

              if (strpos($row['senderId'], ',') != true){
                ...
                $mail->isHTML(true);
                $mail->Subject = 'Resumo de Notícias: '.ucwords($row['alerta']).' ('.(date('d-m-Y')).')';
              ...
              // if (strpos($row['senderId'], ',') == true){
              //   foreach(explode(',',$row['senderId']) as $sendEmail){
              //     $mail->addAddress($sendEmail);
              //   }
              // }else
               $mail->addAddress($row['idusuario']);
                ...
                for($i = 0;$i < count($data['data']['output']['Titulo']);$i++){
                  ...
                  if(strpos($data['data']['output']['data'][$i],'minutos') || strpos($data['data']['output']['data'][$i],'hora') && !strpos($data['data']['output']['data'][$i],'horas')){
                    ...
                    $titulo= highlight($titulo,  $row['alerta']);
            ...
            if($envia == true){
             $mail->send();
              ...
              $upload = "UPDATE alertas SET activado='activo' WHERE id='".$row['id']."'";
                if ($db->query($upload) === TRUE) {
                  echo "Record updated successfully";
                } else {
                  echo "Error updating record: " . $db->error;
